...
volatile struct
{
    ...
    BOOL isSteppingTillClear;
    //when set the user has entered single stepping mode. This is a one thread only thing, so when it's active and another single step happens, discard it
} DebuggerState;


KEVENT debugger_event_WaitForContinue;
//event for kernelmode. Waits till it's set by usermode (usermode function: DBK_Continue_Debug_Event sets it)
...
void debugger_initialize(void)
{
    ...
    Stacks = (PSavedStack*)ExAllocatePool2(POOL_FLAG_PAGED, StackCount * sizeof(PSavedStack),ALLOC_TAG);


    int i;
    for (i = 0; i < StackCount; i++)
    {
        Stacks[i] = (PSavedStack)ExAllocatePool2(POOL_FLAG_PAGED, sizeof(SavedStack),ALLOC_TAG);
...
//called in passive mode
{
    if (Stacks)
    {
        ...
        newStacks = (PSavedStack*)ExAllocatePool2(POOL_FLAG_PAGED, newStackCount * sizeof(PSavedStack),ALLOC_TAG);
        ...
        if (newStacks)
        {
            ...
            {
                newStacks[i] = (PSavedStack)ExAllocatePool2(POOL_FLAG_PAGED, sizeof(SavedStack),ALLOC_TAG);
                if (newStacks[i])
...
VOID debugger_initHookForCurrentCPU_DPC(IN struct _KDPC* Dpc, IN PVOID DeferredContext, IN PVOID SystemArgument1,
...
int interrupt1_handler(UINT_PTR* stackpointer, UINT_PTR* currentdebugregs)
{
    ...
    */
    ...
    if (DebuggerState.globalDebug)
    {
        ...
        if (_dr6.BD)
        {
            ...
            if ((stackpointer[si_cs] & 3) == 0)
            {
                ...
                for (instructionPointer = 0; instruction[instructionPointer] != 0x0f; instructionPointer++);
                //find the start of the instruction, skipping prefixes etc...
                ...
                else if (instruction[instructionPointer + 1] == 0x23)
                {
                    //23=write
                    UINT_PTR gpvalue = 0;
    ...
    if (DebuggerState.isSteppingTillClear)
    //this doesn't really work because when the state comes back to interruptable the system has a critical section lock on the GUI, so yeah... I really need a DBVM display driver for this
    {
        ...
        if (CurrentProcessID == (HANDLE)(UINT_PTR)DebuggerState.debuggedProcessID)
        {
            ...
            if ((((PEFLAGS)&stackpointer[si_eflags])->IF == 0) || (KeGetCurrentIrql() != PASSIVE_LEVEL))
            {
                ...
                else
                {
                    ...
                    DebuggerState.isSteppingTillClear = TRUE;
                    //Just in case a taskswitch happens right after enabling passive level with interrupts
                }
            ...
            if (DebuggerState.globalDebug)
            {
                ...
                if (!DebuggerState.stoppingTheDebugger)
                    //This is set when the driver is unloading. So do NOT set it back then
        ...
        */

            {
                ...
                if (SelectedStackEntry == NULL) //fuck
                    ...
                    rs = breakpointHandler_kernel((UINT_PTR*)(SelectedStackEntry->stacksnapshot), currentdebugregs,
                                                  LBR_Stack, causedbyDBVM);
        ...
        else
        {
            ...
            if (DebuggerState.globalDebug)
            {
                ...
                //real dr6		//fake dr7
                if ((dr6.B0) && (!(dr7.L0 || dr7.G0)))
                {
                    /*DbgPrint("setting RF because of B0\n");*/
                    ((PEFLAGS)&stackpointer[si_eflags])->RF = 1;
                    return 1;
                } //break caused by DR0 and not expected by the current process, ignore this bp and continue
                if ((dr6.B1) && (!(dr7.L1 || dr7.G1)))
                {
                    /*DbgPrint("setting RF because of B1\n");*/
                    ((PEFLAGS)&stackpointer[si_eflags])->RF = 1;
                    return 1;
                } //		...		DR1		...
                if ((dr6.B2) && (!(dr7.L2 || dr7.G2)))
                {
                    /*DbgPrint("setting RF because of B2\n");*/
                    ((PEFLAGS)&stackpointer[si_eflags])->RF = 1;
                    return 1;
                } //		...		DR2		...
                if ((dr6.B3) && (!(dr7.L3 || dr7.G3)))
                {
                    /*DbgPrint("setting RF because of B3\n");*/
                    ((PEFLAGS)&stackpointer[si_eflags])->RF = 1;
                    return 1;
                } //		...		DR3		...
            }
...
int interrupt1_centry(UINT_PTR* stackpointer) //code segment 8 has a 32-bit stackpointer
{
...
#endif
    stackpointer[si_errorcode] = (UINT_PTR)naddress;
    //the errorcode is used as address to call the original function if needed
    ...
    */
    ...
    //make sure the GD bit is disabled (int1 within int1, oooh the fun..., and yes, THIS itself will cause one too)
    ...
    if (DebuggerState.globalDebug)
    //DR's are only accesses when there are DR's(no idea how it handles breakpoints in a different process...), so set them in each thread even those that don't belong original: && (PsGetCurrentProcessId()==(HANDLE)DebuggerState.debuggedProcessID))
    {
        ...
        for (breakpoint = 0; breakpoint < 4; breakpoint++)
        {
            if (DebuggerState.breakpoint[breakpoint].active)
            {
                ...
                while ((debugregister < 4) && (foundone == 0))
                {
                    ...
                    if (((DebuggerState.FakedDebugRegisterState[cpunr()].DR7 >> (debugregister * 2)) & 3) == 0)
                    //DR7.Gx and DR7.Lx are 0
                    {
