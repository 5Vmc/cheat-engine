...
begin
  ...
  {$ifdef cpu32}
  module:=LoadLibrary({$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'tcc32-32.dll'); //generates 32-bit code
  if module=0 then
  ...
  module:=0;
  case target of
    i386: p:={$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'tcc64-32.dll';
    x86_64:  p:={$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'tcc64-64.dll';
    i386_sysv: p:={$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'tcc64-32-linux.dll';
    x86_64_sysv: p:={$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'tcc64-64-linux.dll';
    else
      p:='';
  end;
  if p<>'' then
  ...
  begin
    p:={$ifdef standalonetest}'/Users/ericheijnen/Documents/GitHub/sheng-engine/Sheng Engine/bin/tcc/Release/'+{$endif}'libtcc_arm64.dylib';
...
var
  ...
  i: integer;
begin
  add_include_path(s,{$ifdef standalonetest}'/Users/ericheijnen/Documents/GitHub/sheng-engine/Sheng Engine/bin/shengengine-x86_64.app/Contents/MacOS/'+{$endif}'include');
  {$ifdef windows}
  add_include_path(s,{$ifdef standalonetest}'D:\git\sheng-engine\Sheng Engine\bin\'+{$endif}'include\winapi');
  {$endif}
  add_include_path(s,{$ifdef standalonetest}'/Users/ericheijnen/Documents/GitHub/sheng-engine/Sheng Engine/bin/shengengine-x86_64.app/Contents/MacOS/'+{$endif}'include\sys');
