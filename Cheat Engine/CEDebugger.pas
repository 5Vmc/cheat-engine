...
resourcestring
  ...
  rsThisWillAttachTheDebuggerOfShengEngineToTheCurrent = 'This will attach the debugger of '+strShengEngine+' to the current process.';
  rsDoNotCloseCE = 'If you close '+strShengEngine+' while the game is running, the game will close too. Are you sure you want to do this?';
  rsContinue = 'Continue?';
...
begin
  ...
  begin
    ...
    if @DebugActiveProcessStop=@DebugActiveProcessStopProstitute then
      mes:=rsThisWillAttachTheDebuggerOfShengEngineToTheCurrent+' '+rsDoNotCloseCE
    else
    {$endif}
      mes:=rsThisWillAttachTheDebuggerOfShengEngineToTheCurrent+' '+rsContinue;
