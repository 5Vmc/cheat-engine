...
begin
  ...
  if internet=nil then
    internet:=TWinInternet.Create({$ifdef altname}'Sheng Engine'{$else}strShengEngine{$endif}+' microtransaction system');
  ...
  try
    try
      ...
      MessageDlg('Sorry, but the '+strShengEngine+' server is currently overloaded or you have no internet. Please try again later',mtInformation, [mbok],0);
    end;
  finally
    ss.free;
  end;
