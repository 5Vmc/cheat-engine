...
static int tcc_relocate_ex(TCCState *s1, void *ptr, addr_t ptr_diff)
{
...
#ifdef _WIN64
	if (s1->binary_writer_func == 0)  //Sheng Engine does not put that there
    ...
    for (k = 0; k < 2; ++k) {
        ...
        for(i = 1; i < s1->nb_sections; i++) {
            ...
            if (++f == 1 && align < run_section_alignment) //sheng engine modification:  RUN_SECTION_ALIGNMENT->run_section_alignment
    ...
    for(i = 1; i < s1->nb_sections; i++) {
		...
		{
			//sheng engine binary writer addition start
		    ...
		    //sheng engine binary writer addition stop
				memset(ptr, 0, length);
		}
		else
		{
...
static void set_pages_executable(TCCState *s1, void *ptr, unsigned long length)
{
  //Sheng Engine modification:  This code is not needed
  return;
  //Sheng Engine modification end
