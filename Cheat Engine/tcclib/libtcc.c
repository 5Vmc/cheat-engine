...
static int tcc_compile(TCCState *s1, int filetype, const char *str, int fd)
{
    ...
    if (setjmp(s1->error_jmp_buf) == 0) {
        ...
        if (fd == -1) {
			...
			//Sheng Engine <string> counter Start
			char sourcename[200];
			snprintf(sourcename, 200, "<string-%d>", s1->stringcompiles);
			s1->stringcompiles++;
			//Sheng Engine <string> counter Stop

            tcc_open_bf(s1, sourcename, len);   //Sheng Engine <string> counter -> "<string>" -> sourcename 
...
#ifdef TCC_IS_NATIVE
    /* free runtime memory */
	if (0 == s1->binary_writer_func) //sheng engine: Don't bother freeing if a binary writer function is set (CE will do that)
      tcc_run_free(s1);
#endif
